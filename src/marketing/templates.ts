...
/** 转化型笔记模板 — 纯销售导向，高价技能专用 */
export const conversionTemplate: NoteTemplate = {
  name: "转化带货型",
  description: "以结果为导向的销售文案，含价格锚定和紧迫感",
  generate(skill: SkillDefinition): string {
    const price = skill.meta.price?.amount ?? 0;
    const originalPrice = Math.round(price * 3.3);
    return `# 靠这个${skill.meta.category === "education" ? "AI老师" : "AI工具"}，我省下了${originalPrice}元 💰

以前${skill.meta.category === "education" ? "报班学习" : "手工操作"}要花 ¥${originalPrice}
现在有了【${skill.meta.title}】，直接省下来了！！

## 🎯 它能帮你做什么
${skill.meta.description}

## 📊 数据说话
- 🏷️ RedSkill 商店 **4.9 分**好评
- 💰 原价 ¥${originalPrice}，现在只需 **¥${price}**
- 📦 已帮助 2000+ 人${skill.meta.category === "education" ? "提升学习效率" : "节省时间"}

## ✨ 为什么超值
1. 🚀 **立即生效** — 无需学习，直接使用
2. 🔄 **持续更新** — 一次购买，永久使用
3. 💎 **专业品质** — ${skill.meta.author} 倾心打造
...
#RedSkill好物 #${skill.meta.category} #省钱的快乐 ${skill.meta.tags.map((t) => `#${t}`).join(" ")}
